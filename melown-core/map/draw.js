...
Melown.Map.prototype.drawTileInfo = function(tile_, node_, cameraPos_, mesh_, pixelSize_) {
    ...
    pos_[2] = pos_[2] * 0.9999;

    //draw lods
    if (this.drawLods_ == true) {
        text_ = "" + tile_.id_[0];
        this.renderer_.drawText(Math.round(pos_[0]-(text_.length*4)*0.5), Math.round(pos_[1]-4), 4, text_, [255,0,0,255], pos_[2]);
    }

    //draw indices
    if (this.drawIndices_ == true) {
        var text_ = "" + tile_.id_[1] + " " + tile_.id_[2];
        this.renderer_.drawText(Math.round(pos_[0]-(text_.length*4)*0.5), Math.round(pos_[1]-11), 4, text_, [0,255,255,255], pos_[2]);
    }

    //draw positions
    if (this.drawPositions_ == true) {
        var text_ = "" + min_[0].toFixed(1) + " " + min_[1].toFixed(1) + " " + min_[2].toFixed(1);
        this.renderer_.drawText(Math.round(pos_[0]-(text_.length*4)*0.5), Math.round(pos_[1]+3), 4, text_, [0,255,255,255], pos_[2]);
    }

    //draw face count
    if (this.drawFaceCount_ == true && mesh_ != null) {
        var text_ = "" + mesh_.faces_ + " - " + mesh_.submeshes_.length;
        this.renderer_.drawText(Math.round(pos_[0]-(text_.length*4)*0.5), Math.round(pos_[1]+10), 4, text_, [0,255,0,255], pos_[2]);
    }

    //draw distance
    if (this.drawDistance_ == true) {
        var text_ = "" + pixelSize_[1].toFixed(2) + "  " + pixelSize_[0].toFixed(2) + "  " + node_.pixelSize_.toFixed(2);
        this.renderer_.drawText(Math.round(pos_[0]-(text_.length*4)*0.5), Math.round(pos_[1]+17), 4, text_, [255,0,255,255], pos_[2]);
    }
