...
RendererDraw.prototype.drawLineString2 = function(points, screenSpace, size, color, depthOffset, depthTest, transparent, writeDepth, useState) {
    var gpu = this.gpu;
    var gl = this.gl;
    var renderer = this.renderer;
    var index = 0, p, i;
    ...
    if (screenSpace) { 
        ...
        var mp = renderer.camera.getProjectionMatrix();
        var near = renderer.camera.getNear() + 0.1 + 30000;
        ...
        var prog = renderer.progLine4;
        var clipped = false, pp = [0,0,0,0], dir = [0,0,0], l, p2, culled = false;

        for (i = 0; i < totalPoints; i++) {
            p = points[i];
            p = mat4.multiplyVec4(mv, [p[0] - cameraPos[0], p[1] - cameraPos[1], p[2] - cameraPos[2], 1 ]); 
            ...
            if (p[2] > -near) {
                ...
                p2 = points[i+1];
                p2 = mat4.multiplyVec4(mv, [p2[0] - cameraPos[0], p2[1] - cameraPos[1], p2[2] - cameraPos[2], 1 ]); 

                l = vec3.normalize4([p2[0] - p[0], p2[1] - p[1], p2[2] - p[2]], dir);
            ...
            mat4.multiplyVec4(mp, p, pp); 

            //project point coords to screen
            if (!culled) {
                if (p[3] != 0) {
                    //x and y are in screen pixels
                    plineBuffer[index] = ((pp[0]/pp[3])+1.0)*0.5*curSize[0];
                    plineBuffer[index+1] = (-(pp[1]/pp[3])+1.0)*0.5*curSize[1]; 
                    plineBuffer[index+2] = pp[2]/pp[3]; //depth in meters

                    //this.drawText(plineBuffer[index], plineBuffer[index+1], 4*renderer.debug.debugTextSize, ''+p[0].toFixed(0)+' '+p[1].toFixed(0)+' '+p[2].toFixed(0) , [1,0,0,1], 0.5);
